...
const Result = ({ isSuccesss, datas }: any) => {
  useEffect(() => {
  ...
  return (
    <>
      ...
      <Content bg="dark">
              ...
              <Flex direction="column" gap={16}>
                ...
                <Flex direction="column">
                  <Text typo="Body2" color="White">
                    {names[data.data.celebrityInitial]}
                  </Text>
                  <Text typo="Heading" color="White">
                    {Math.round(data.data.accuracy * 100) + '%'}
                  </Text>
                </Flex>
                <ImgWrapper>
                  <Img src={`/img/drama/${data.data.celebrityInitial}.png`} />
                </ImgWrapper>
              </Flex>
              <Flex direction="column" gap={16}>
                <Text typo="Heading" color="Yellow2">
                ...
                <ImgWrapper>
                  <Img src={`/img/luxury_icon/${data.data.celebrityInitial}.png`} />
                </ImgWrapper>
              </Flex>
            </StyledFlex>
          </>
        ...
        <Button fullWidth text="테스트 다시 하기" varient="yellow" color="Gray3" onClick={() => router.push('/test')} />
...
const Content = styled.div<{ bg: string }>`
  ...
  background: ${({ bg }) => (bg == 'dark' ? ({ theme }) => theme.palette.Gray3 : ({ theme }) => theme.palette.Yellow2)};
